...
contract RipToken is ERC20("RipToken", "Rip"), Ownable {
    ...
    function mint(address _to, uint256 _amount) public onlyOwner {
        require(
            ...
            "Rip::mint: cannot exceed max supply"
        );
        _mint(_to, _amount);
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "Rip::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "Rip::delegateBySig: invalid nonce"
        );
        ...
        returns (uint256)
    {
        require(
            ...
            "Rip::getPriorVotes: not yet determined"
        );
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying Rips (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "Rip::_writeCheckpoint: block number exceeds 32 bits"
        );
...
// File: contracts/MasterChefRipV2.sol
...
// MasterChefRip is a boss. He says "go f your blocks lego boy, I'm gonna use timestamp instead".
...
// will be transferred to a governance smart contract once Rip is sufficiently
// distributed and the community can show to govern itself.
//
// With thanks to the Lydia Finance team.
//
// Godspeed and may the 10x be with you.
contract MasterChefRipV2 is Ownable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of Rips
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accRipPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accRipPerShare` (and `lastRewardTimestamp`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. Rips to distribute per second.
        uint256 lastRewardTimestamp; // Last timestamp that Rips distribution occurs.
        uint256 accRipPerShare; // Accumulated Rips per share, times 1e12. See below.
        IRewarder rewarder;
    }
    ...
    // Rip tokens created per second.
    uint256 public RipPerSec;
    ...
    // The timestamp when Rip mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 _RipPerSec);
    ...
    // Update the given pool's Rip allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        IRewarder _rewarder,
    ...
    // View function to see pending Rips on frontend.
        ...
        )
    {
        ...
        if (block.timestamp > pool.lastRewardTimestamp && lpSupply != 0) {
            ...
            uint256 lpPercent = 1000 -
                ...
                treasuryPercent -
                investorPercent;
            uint256 RipReward = multiplier
            .mul(RipPerSec)
            .mul(pool.allocPoint)
            .div(totalAllocPoint)
            .mul(lpPercent)
            .div(1000);
            accRipPerShare = accRipPerShare.add(
                RipReward.mul(1e12).div(lpSupply)
            );
        }
        pendingRip = user.amount.mul(accRipPerShare).div(1e12).sub(
            user.rewardDebt
        );
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = block.timestamp.sub(pool.lastRewardTimestamp);
        uint256 RipReward = multiplier.mul(RipPerSec).mul(pool.allocPoint).div(
            totalAllocPoint
        );
        uint256 lpPercent = 1000 -
            devPercent -
            treasuryPercent -
            investorPercent;
        Rip.mint(devAddr, RipReward.mul(devPercent).div(1000));
        Rip.mint(treasuryAddr, RipReward.mul(treasuryPercent).div(1000));
        Rip.mint(investorAddr, RipReward.mul(investorPercent).div(1000));
        Rip.mint(address(this), RipReward.mul(lpPercent).div(1000));
        pool.accRipPerShare = pool.accRipPerShare.add(
            RipReward.mul(1e12).div(lpSupply).mul(lpPercent).div(1000)
        );
        pool.lastRewardTimestamp = block.timestamp;
        emit UpdatePool(
            _pid,
    ...
    // Deposit LP tokens to MasterChef for Rip allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            .div(1e12)
            .sub(user.rewardDebt);
            safeRipTransfer(msg.sender, pending);
            emit Harvest(msg.sender, _pid, pending);
        }
        user.amount = user.amount.add(_amount);
        user.rewardDebt = user.amount.mul(pool.accRipPerShare).div(1e12);

        IRewarder rewarder = poolInfo[_pid].rewarder;
        if (address(rewarder) != address(0)) {
            rewarder.onRipReward(msg.sender, user.amount);
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        // Harvest Rip
        uint256 pending = user.amount.mul(pool.accRipPerShare).div(1e12).sub(
            user.rewardDebt
        );
        safeRipTransfer(msg.sender, pending);
        emit Harvest(msg.sender, _pid, pending);
    ...
    // Safe Rip transfer function, just in case if rounding error causes pool to not have enough Rips.
    function safeRipTransfer(address _to, uint256 _amount) internal {
        uint256 RipBal = Rip.balanceOf(address(this));
        if (_amount > RipBal) {
            Rip.transfer(_to, RipBal);
        } else {
            Rip.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _RipPerSec) public onlyOwner {
        massUpdatePools();
        RipPerSec = _RipPerSec;
        emit UpdateEmissionRate(msg.sender, _RipPerSec);
    }
}
