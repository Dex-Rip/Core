...
// RipMakerV3 is MasterRip's left hand and kinda a wizard. He can cook up Rip from pretty much anything!
// This contract handles "serving up" rewards for xRip holders by trading tokens collected from fees for Rip.

// T1 - T4: OK
contract RipMakerV3 is Ownable {
    ...
    modifier onlyAuth() {
        require(isAuth[msg.sender] || anyAuth, "RipMakerV3: FORBIDDEN");
        _;
    }
    ...
    function setBridge(address token, address bridge) external onlyOwner {
        // Checks
        require(token != Rip && token != wavax && token != bridge, "RipMakerV3: Invalid bridge");
    ...
    modifier onlyEOA() {
        ...
        require(msg.sender == tx.origin, "RipMakerV3: must use EOA");
        _;
    }
    ...
    function _convert(address token0, address token1) internal {
        ...
        if (token0 == token1) {
            ...
            require(address(pair) != address(0), "RipMakerV3: Invalid pair");
    ...
    // F1 - F10: OK
    // C1 - C24: OK
    // All safeTransfer, _swap, _toRip, _convertStep: X1 - X5: OK
    ...
    function _swap(
        ...
        require(address(pair) != address(0), "RipMakerV3: Cannot convert");
    ...
    function _toRip(address token, uint256 amountIn) internal returns (uint256 amountOut) {
        ...
        amountOut = _swap(token, Rip, amount, bar);
    }
    ...
    function getAmountOut(
        ...
        require(amountIn > 0, "RipMakerV3: INSUFFICIENT_INPUT_AMOUNT");
        require(reserveIn > 0 && reserveOut > 0, "RipMakerV3: INSUFFICIENT_LIQUIDITY");
