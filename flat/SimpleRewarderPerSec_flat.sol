...
/**
 * This is a sample contract to be used in the MasterChefRip contract for partners to reward
 * stakers with their native token alongside Rip.
 *
 * It assumes no minting rights, so requires a set amount of YOUR_TOKEN to be transferred to this contract prior.
 * E.g. say you've allocated 100,000 XYZ to the Rip-XYZ farm over 30 days. Then you would need to transfer
...
contract SimpleRewarderPerSec is IRewarder, BoringOwnable, ReentrancyGuard {
    ...
    IMasterChefRip public immutable MCJ;
    ...
    modifier onlyMCJ() {
        require(msg.sender == address(MCJ), "onlyMCJ: only MasterChefRip can call this function");
        _;
    }
    ...
    constructor(
        ...
        require(Address.isContract(address(_MCJ)), "constructor: MasterChefRip must be a valid contract");
    ...
    /// @notice Function called by MasterChefRip whenever staker claims Rip harvest. Allows staker to also receive a 2nd reward token.
    /// @param _user Address of user
    /// @param _lpAmount Number of LP tokens the user has
    function onRipReward(address _user, uint256 _lpAmount) external override onlyMCJ nonReentrant {
        updatePool();
