...
describe("MasterChefRip", function () {
  ...
  it("should revert contract creation if dev and treasury percents don't meet criteria", async function () {
    ...
    // dev percent failure
    await expect(
      this.MasterChef.deploy(this.Rip.address, this.dev.address, this.treasury.address, "100", startTime, "1100", this.treasuryPercent)
    ).to.be.revertedWith("constructor: invalid dev percent value")

    // Invalid treasury percent failure
    await expect(
      this.MasterChef.deploy(this.Rip.address, this.dev.address, this.treasury.address, "100", startTime, this.devPercent, "1100")
    ).to.be.revertedWith("constructor: invalid treasury percent value")

    // Sum of dev and treasury precent too high
    await expect(
      this.MasterChef.deploy(this.Rip.address, this.dev.address, this.treasury.address, "100", startTime, "500", "501")
  ...
  context("With ERC/LP token added to the field", function () {
    ...
    it("should give out Rips only after farming time", async function () {
      ...
      await this.Rip.transferOwnership(this.chef.address) // t-58
      await this.chef.setDevPercent(this.devPercent) // t-57
      await this.chef.setTreasuryPercent(this.treasuryPercent) // t-56
      ...
      expect(await this.Rip.balanceOf(this.bob.address)).to.be.within(600, 660)
      ...
      expect(await this.Rip.balanceOf(this.bob.address)).to.be.within(900, 960)
    ...
    it("should not distribute Rips if no one deposit", async function () {
      ...
      expect(await this.Rip.balanceOf(this.bob.address)).to.be.within(720, 780)
    ...
    it("should distribute Rips properly for each staker", async function () {
      ...
      expect(await this.Rip.balanceOf(this.alice.address)).to.be.within(340 - this.tokenOffset, 400 + this.tokenOffset)
      expect(await this.Rip.balanceOf(this.bob.address)).to.equal("0")
      ...
      expect(await this.Rip.balanceOf(this.chef.address)).to.be.within(260 - this.tokenOffset, 360 + this.tokenOffset)
      ...
      expect(await this.Rip.balanceOf(this.alice.address)).to.be.within(340 - this.tokenOffset, 400 + this.tokenOffset)
      expect(await this.Rip.balanceOf(this.bob.address)).to.be.within(371 - this.tokenOffset, 431 + this.tokenOffset)
      expect(await this.Rip.balanceOf(this.carol.address)).to.equal("0")
      ...
      expect(await this.Rip.balanceOf(this.chef.address)).to.be.within(489 - this.tokenOffset, 589 + this.tokenOffset)
      ...
      expect(await this.Rip.balanceOf(this.alice.address)).to.be.within(696 - this.tokenOffset, 756 + this.tokenOffset)
      // Bob should have: 371 + 10*1.5/6.5*60 + 10*1.5/4.5*60 = 709 (+60)
      expect(await this.Rip.balanceOf(this.bob.address)).to.be.within(709 - this.tokenOffset, 769 + this.tokenOffset)
      // Carol should have: 2*3/6*60 + 10*3/7*60 + 10*3/6.5*60 + 10*3/4.5*60 + 10*60 = 1594 (+60)
      expect(await this.Rip.balanceOf(this.carol.address)).to.be.within(1594 - this.tokenOffset, 1654 + this.tokenOffset)
    ...
    it("should give proper Rips allocation to each pool", async function () {
      ...
      expect(await this.chef.pendingRip(0, this.alice.address)).to.be.within(600 - this.tokenOffset, 660 + this.tokenOffset)
      ...
      expect(await this.chef.pendingRip(0, this.alice.address)).to.be.within(700 - this.tokenOffset, 760 + this.tokenOffset)
      ...
      expect(await this.chef.pendingRip(0, this.alice.address)).to.be.within(800 - this.tokenOffset, 860 + this.tokenOffset)
      expect(await this.chef.pendingRip(1, this.bob.address)).to.be.within(200 - this.tokenOffset, 260 + this.tokenOffset)
      // Alice and Bob should not have pending rewards in pools they're not staked in
      expect(await this.chef.pendingRip(1, this.alice.address)).to.equal("0")
      expect(await this.chef.pendingRip(0, this.bob.address)).to.equal("0")
      ...
      expect(await this.Rip.balanceOf(this.alice.address)).to.be.within(820 - this.tokenOffset, 880 + this.tokenOffset)
      await this.chef.connect(this.bob).withdraw(1, "5", { from: this.bob.address }) // t+32
      // Bob should have: 200 + 2*2/3*60 = 280 (+60)
      expect(await this.Rip.balanceOf(this.bob.address)).to.be.within(280 - this.tokenOffset, 340 + this.tokenOffset)
    })
    ...
    it("should give proper Rips after updating emission rate", async function () {
      ...
      expect(await this.chef.pendingRip(0, this.alice.address)).to.be.within(6000, 6060)
      // Lower emission rate to 40 Rip per sec
      ...
      expect(await this.chef.pendingRip(0, this.alice.address)).to.be.within(6156, 6216)
    })
  })
