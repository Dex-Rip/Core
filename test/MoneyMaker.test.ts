...
describe("moneyMaker", function () {
  ...
  describe("setBridge", function () {
    ...
    it("does not allow to set bridge for WAVAX", async function () {
      await expect(this.moneyMaker.setBridge(this.wavax.address, this.Rip.address)).to.be.revertedWith("MoneyMaker: Invalid bridge")
    })
    ...
    it("emits correct event on bridge", async function () {
      await expect(this.moneyMaker.setBridge(this.dai.address, this.Rip.address))
        .to.emit(this.moneyMaker, "LogBridgeSet")
        .withArgs(this.dai.address, ethers.constants.AddressZero, this.Rip.address)
    })
  })
  ...
  describe("convert Pairs", function () {
    ...
    it("Should convert Rip - AVAX above slippage", async function () {
      await this.zap.zapIn(this.RipAvax.address, { value: "2000000000000000000" })

      const RipAvax = await getPairInfo(this.RipAvax, this.dev.address)
      const avaxUsdc = await getPairInfo(this.avaxUsdc, this.dev.address)

      const [RipAmount, avaxAmount] =
        this.Rip.address == RipAvax.token0 ? [RipAvax.amount0, RipAvax.amount1] : [RipAvax.amount1, RipAvax.amount0]

      const swappedRipToAvax = swapTo(RipAvax, (await RipAvax.token0) == this.Rip.address, RipAmount, "9900")
      const swappedAvaxToUsdc = swapTo(avaxUsdc, (await avaxUsdc.token0) == this.wavax.address, avaxAmount.add(swappedRipToAvax), "9900")

      await this.RipAvax.transfer(this.moneyMaker.address, await this.RipAvax.balanceOf(this.dev.address))
      await this.moneyMaker.convert(this.Rip.address, this.wavax.address, "100")

      expect(await this.usdc.balanceOf(this.moneyMaker.address)).to.equal(0)
      expect(await this.RipAvax.balanceOf(this.moneyMaker.address)).to.equal(0)
    ...
    it("reverts if slippage is lower than 0.3% (fees for swap)", async function () {
      await this.zap.zapIn(this.RipAvax.address, { value: "2000000000000000000" })
      await this.RipAvax.transfer(this.moneyMaker.address, await this.RipAvax.balanceOf(this.dev.address))
      await expect(this.moneyMaker.convert(this.Rip.address, this.wavax.address, "29")).to.be.revertedWith("MoneyMaker: Slippage caught") // slippage at 0.29% this will revert as the fee for a swap is 0.3%
    })
    ...
    it("reverts if using a pair with really low liquidity even with slippage at maximum", async function () {
      ...
      await this.mimTime.transfer(this.moneyMaker.address, await this.mimTime.balanceOf(this.dev.address))
      await this.moneyMaker.setBridge(this.mim.address, this.Rip.address)
      await expect(this.moneyMaker.convert(this.mim.address, this.time.address, "4999")).to.be.revertedWith("MoneyMaker: Slippage caught") // slippage at 1% this will revert as the Rip/MIM pair has really low liquidity.
    })
    ...
    it("reverts if it loops back", async function () {
      await this.zap.zapIn(this.RipUsdt.address, { value: ethers.utils.parseEther("2") })
      await this.RipUsdt.transfer(this.moneyMaker.address, await this.RipUsdt.balanceOf(this.dev.address))
      await this.moneyMaker.setBridge(this.Rip.address, this.usdt.address)
      await this.moneyMaker.setBridge(this.usdt.address, this.Rip.address)
      await expect(this.moneyMaker.convert(this.usdt.address, this.Rip.address, "100")).to.be.reverted
    })
  ...
  describe("convertMultiple", function () {
    it("should allow to convert multiple", async function () {
      await this.zap.zapIn(this.RipAvax.address, { value: ethers.utils.parseEther("2") })
      await this.zap.zapIn(this.avaxUsdc.address, { value: ethers.utils.parseEther("2") })
      await this.RipAvax.transfer(this.moneyMaker.address, await this.RipAvax.balanceOf(this.dev.address))
      await this.avaxUsdc.transfer(this.moneyMaker.address, await this.avaxUsdc.balanceOf(this.dev.address))
      await this.moneyMaker.convertMultiple([this.Rip.address, this.usdc.address], [this.wavax.address, this.wavax.address], "100")
      expect(await this.usdc.balanceOf(this.moneyMaker.address)).to.equal(0)
      expect(await this.RipAvax.balanceOf(this.moneyMaker.address)).to.equal(0)
      expect(await this.avaxUsdc.balanceOf(this.moneyMaker.address)).to.equal(0)
  ...
  describe("setToken", function () {
    it("should convert Rip - AVAX to Rip", async function () {
      this.moneyMaker.setTokenToAddress(this.Rip.address)
      ...
      let previousBalance = await this.Rip.balanceOf(BAR_ADDRESS)
      await this.zap.zapIn(this.avaxUsdc.address, { value: ethers.utils.parseEther("2") })
      expect(await this.Rip.balanceOf(BAR_ADDRESS)).to.equal(previousBalance)

      await this.avaxUsdc.transfer(this.RipMaker.address, (await this.avaxUsdc.balanceOf(this.dev.address)).div(2))
      await this.RipMaker.convert(this.usdc.address, this.wavax.address)
      expect((await this.Rip.balanceOf(BAR_ADDRESS)).sub(previousBalance.add(ethers.utils.parseEther("50")))).to.be.above(0)

      previousBalance = await this.Rip.balanceOf(BAR_ADDRESS)
      await this.avaxUsdc.transfer(this.moneyMaker.address, await this.avaxUsdc.balanceOf(this.dev.address))
      await this.moneyMaker.convert(this.usdc.address, this.wavax.address, "100")
      expect((await this.Rip.balanceOf(BAR_ADDRESS)).sub(previousBalance.add(ethers.utils.parseEther("50")))).to.be.above(0)

      expect(await this.Rip.balanceOf(this.RipMaker.address)).to.equal(0)
      expect(await this.avaxUsdc.balanceOf(this.RipMaker.address)).to.equal(0)
      expect(await this.avaxUsdc.balanceOf(this.moneyMaker.address)).to.equal(0)
    })
  })
