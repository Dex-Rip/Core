...
describe("Stable Rip Staking", function () {
  before(async function () {
    this.StableRipStakingCF = await ethers.getContractFactory(
      "StableRipStaking"
    );
    ...
    this.RipMaker = this.signers[4];
    this.penaltyCollector = this.signers[5];
  });

  beforeEach(async function () {
    this.rewardToken = await this.RipTokenCF.deploy();
    ...
    await this.rewardToken.mint(
      this.RipMaker.address,
      ethers.utils.parseEther("1000000")
    ); // 1_000_000 tokens

    this.stableRipStaking = await hre.upgrades.deployProxy(
      this.StableRipStakingCF,
      [
  ...
  describe("should allow deposits and withdraws", function () {
    it("should allow deposits and withdraws of multiple users", async function () {
      ...
      expect(
        await this.Rip.balanceOf(this.stableRipStaking.address)
      ).to.be.equal(ethers.utils.parseEther("97"));
      // 100 * 0.97 = 97
      expect(
        (
          await this.stableRipStaking.getUserInfo(
            this.alice.address,
            this.Rip.address
          )
        )[0]
      ...
      expect(await this.Rip.balanceOf(this.bob.address)).to.be.equal(
      ...
      expect(await this.Rip.balanceOf(this.carol.address)).to.be.equal(
      ...
      await this.stableRipStaking.connect(this.bob).withdraw("1");
      expect(await this.Rip.balanceOf(this.bob.address)).to.be.equal(
    ...
    it("should update variables accordingly", async function () {
      await this.stableRipStaking.connect(this.alice).deposit("1");

      await this.rewardToken
        .connect(this.RipMaker)
        .transfer(this.stableRipStaking.address, ethers.utils.parseEther("1"));
      expect(
        await this.rewardToken.balanceOf(this.stableRipStaking.address)
      ).to.be.equal(ethers.utils.parseEther("1"));
      expect(
        await this.stableRipStaking.lastRewardBalance(this.rewardToken.address)
      ).to.be.equal("0");

      await increase(86400);
      expect(
        await this.stableRipStaking.pendingReward(
          this.alice.address,
          this.rewardToken.address
        )
      ).to.be.equal(ethers.utils.parseEther("1"));
      ...
      await this.stableRipStaking.updateReward(this.rewardToken.address);
      expect(
        await this.stableRipStaking.pendingReward(
          this.alice.address,
          this.rewardToken.address
    ...
    it("should allow deposits and withdraws of multiple users and distribute rewards accordingly", async function () {
      ...
      await this.rewardToken
        .connect(this.RipMaker)
        .transfer(this.stableRipStaking.address, ethers.utils.parseEther("6"));
      await this.stableRipStaking.updateReward(this.rewardToken.address);
      await increase(86400);
      ...
      await this.stableRipStaking.connect(this.bob).withdraw("0");
      // reward = accRewardBalance * carolShare / PRECISION
    ...
    it("should distribute token accordingly even if update isn't called every day", async function () {
      ...
      await this.rewardToken
        ...
        .transfer(this.stableRipStaking.address, ethers.utils.parseEther("1"));
      await increase(86400);
      await this.stableRipStaking.connect(this.alice).withdraw(0);
    ...
    it("should allow deposits and withdraws of multiple users and distribute rewards accordingly even if someone enters or leaves", async function () {
      ...
      await this.rewardToken
        .connect(this.RipMaker)
        .transfer(this.stableRipStaking.address, ethers.utils.parseEther("4"));
      await increase(86400);
    ...
    it("pending tokens function should return the same number of token that user actually receive", async function () {
      ...
      ); // We send 100 Tokens to sRip's address

      const pendingReward = await this.stableRipStaking.pendingReward(
        this.alice.address,
        this.rewardToken.address
      );
      await this.stableRipStaking.connect(this.alice).withdraw("0"); // Alice shouldn't receive any token of the last reward
      expect(await this.Rip.balanceOf(this.alice.address)).to.be.equal(
        ethers.utils.parseEther("700")
      );
      expect(await this.rewardToken.balanceOf(this.alice.address)).to.be.equal(
    ...
    it("should allow rewards in Rip and USDC", async function () {
      await this.stableRipStaking
        .connect(this.alice)
      ...
      // accRipBalance = 0
      // reward = 0
      expect(await this.Rip.balanceOf(this.alice.address)).to.be.equal(0);

      await this.stableRipStaking.addRewardToken(this.Rip.address);
      await this.Rip.mint(
        this.stableRipStaking.address,
      ...
      // accRipBalance = RipBalance * PRECISION / totalStaked
      //                  = 6e18 * 1e24 / 291e18
      //                  = 0.002061855670103092783505e24
      // reward = accRipBalance * aliceShare / PRECISION
      //        = accRipBalance * 970e18 / 1e24
      //        = 1.999999999999999999e18
      expect(await this.Rip.balanceOf(this.bob.address)).to.be.closeTo(
        ethers.utils.parseEther("2"),
        ethers.utils.parseEther("0.0001")
      );
      ...
      //        = accRipBalance * 970e18 / 1e24
      //        = 1.999999999999999999e18
      expect(await this.Rip.balanceOf(this.alice.address)).to.be.closeTo(
        ethers.utils.parseEther("2"),
        ethers.utils.parseEther("0.0001")
      );
    });
    ...
    it("rewardDebt should be updated as expected, alice deposits before last reward is sent", async function () {
      let token1 = await this.RipTokenCF.deploy();
      await this.stableRipStaking.addRewardToken(token1.address);

      await this.stableRipStaking.connect(this.alice).deposit(1);
      await this.stableRipStaking.connect(this.bob).deposit(1);

      await token1.mint(
        this.stableRipStaking.address,
        ethers.utils.parseEther("1")
      );
      await this.stableRipStaking.connect(this.alice).withdraw(1);
      ...
      await this.stableRipStaking.removeRewardToken(token1.address);
    });
    ...
    it("should allow adding and removing a rewardToken, only by owner", async function () {
      let token1 = await this.RipTokenCF.deploy();
      await expect(
        this.stableRipStaking.connect(this.alice).addRewardToken(token1.address)
      ).to.be.revertedWith("Ownable: caller is not the owner");
      expect(
        await this.stableRipStaking.isRewardToken(token1.address)
      ).to.be.equal(false);
      expect(await this.stableRipStaking.rewardTokensLength()).to.be.equal(1);

      await this.stableRipStaking
        .connect(this.dev)
        .addRewardToken(token1.address);
      await expect(
        this.stableRipStaking.connect(this.dev).addRewardToken(token1.address)
      ).to.be.revertedWith("StableRipStaking: token can't be added");
      expect(
        await this.stableRipStaking.isRewardToken(token1.address)
      ).to.be.equal(true);
      expect(await this.stableRipStaking.rewardTokensLength()).to.be.equal(2);
    ...
    it("should allow setting a new deposit fee, only by owner", async function () {
      ...
      expect(
        await this.Rip.balanceOf(this.penaltyCollector.address)
      ).to.be.equal(ethers.utils.parseEther("3"));

      await expect(
        this.stableRipStaking.connect(this.alice).setDepositFeePercent("0")
      ).to.be.revertedWith("Ownable: caller is not the owner");
      await expect(
        this.stableRipStaking
          .connect(this.dev)
          .setDepositFeePercent(ethers.utils.parseEther("0.5").add("1"))
      ).to.be.revertedWith(
        "StableRipStaking: deposit fee can't be greater than 50%"
      );

      await this.stableRipStaking
        .connect(this.dev)
        .setDepositFeePercent(ethers.utils.parseEther("0.49"));
      expect(await this.stableRipStaking.depositFeePercent()).to.be.equal(
    ...
    it("should allow emergency withdraw", async function () {
      ...
      await this.stableRipStaking.connect(this.alice).emergencyWithdraw(); // Alice shouldn't receive any token of the last reward
      expect(await this.Rip.balanceOf(this.alice.address)).to.be.equal(
      ...
      const userInfo = await this.stableRipStaking.getUserInfo(
        this.stableRipStaking.address,
        this.rewardToken.address
      );
