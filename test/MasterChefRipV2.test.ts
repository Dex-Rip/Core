...
describe("MasterChefRipV2", function () {
  ...
  it("should revert contract creation if dev and treasury percents don't meet criteria", async function () {
    ...
    await expect(
      this.MCV2.deploy(this.Rip.address, this.dev.address, this.treasury.address, this.investor.address, "100", startTime, "300", "300", "401")
  ...
  it("should set correct state variables", async function () {
    ...
    expect(Rip).to.equal(this.Rip.address)
  ...
  context("With ERC/LP token added to the field and using SimpleRewarderPerBlock", function () {
    ...
    it("should check rewarder's arguments are contracts", async function () {
      ...
      ).to.be.revertedWith("constructor: MasterChefRipV2 must be a valid contract")
    })
    ...
    it("should check rewarder added and set properly", async function () {
      ...
      this.rewarder = await this.SimpleRewarderPerBlock.deploy(this.Rip.address, this.lp.address, this.partnerRewardPerBlock, this.chef.address)
    ...
    it("should allow emergency withdraw from MasterChefRipV2", async function () {
      const startTime = (await latest()).add(60)
      this.chef = await this.MCV2.deploy(
        this.Rip.address,
        this.dev.address,
    ...
    it("should reward partner token accurately after rewarder runs out of tokens and is topped up again", async function () {
      ...
      await this.Rip.transferOwnership(this.chef.address) // t-56, b=17
    ...
    it("should only allow MasterChefRipV2 to call onRipReward", async function () {
      const startTime = (await latest()).add(60)
      this.chef = await this.MCV2.deploy(
        this.Rip.address,
        this.dev.address,
      ...
      await expect(this.rewarder.onRipReward(this.bob.address, "100")).to.be.revertedWith("onlyMCV2: only MasterChef V2 can call this function") // t-10, b=24
      await this.chef.connect(this.bob).deposit(0, "0") // t-9, b=25
      // Bob should have:
      //   - 0 RipToken
      //   - 3*40 = 80 PartnerToken
      expect(await this.Rip.balanceOf(this.bob.address)).to.equal("0")
    ...
    it("should allow rewarder to be set and removed mid farming", async function () {
      ...
      //   - 10*50 = 500 (+50) RipToken
      //   - 0 PartnerToken
      await advanceTimeAndBlock(20) // t+10, b=25
      expect((await this.chef.pendingTokens(0, this.bob.address)).pendingRip).to.be.within(500, 550)
      ...
      //   - 500 + 10*50 = 1000 (+50) RipToken
      //   - 0 PartnerToken
      await advanceTimeAndBlock(9) // t+20, b=27
      expect((await this.chef.pendingTokens(0, this.bob.address)).pendingRip).to.be.within(1000, 1050)
      expect((await this.chef.pendingTokens(0, this.bob.address)).bonusTokenAddress).to.equal(ADDRESS_ZERO)
      expect((await this.chef.pendingTokens(0, this.bob.address)).pendingBonusToken).to.equal(0)
      ...
      expect((await this.chef.pendingTokens(0, this.bob.address)).pendingRip).to.be.within(1500, 1550)
      ...
      //   - 9*50 = 450 (+50) RipToken
      //   - 2*40 = 80 PartnerToken
      await advanceTimeAndBlock(4) // t+35, b=32
      await advanceTimeAndBlock(5) // t+40, b=33
      expect((await this.chef.pendingTokens(0, this.bob.address)).pendingRip).to.be.within(450, 500)
      expect((await this.chef.pendingTokens(0, this.bob.address)).bonusTokenAddress).to.equal(this.partnerToken.address)
      expect((await this.chef.pendingTokens(0, this.bob.address)).bonusTokenSymbol).to.equal("SUSHI")
      ...
      //   - 450 + 10*50 = 950 (+50) RipToken
      //   - 80 + 1*40 = 120 PartnerToken
      await advanceTimeAndBlock(4) // t+45, b=35
      await advanceTimeAndBlock(5) // t+50, b=36
      expect((await this.chef.pendingTokens(0, this.bob.address)).pendingRip).to.be.within(950, 1000)
      expect((await this.chef.pendingTokens(0, this.bob.address)).bonusTokenAddress).to.equal(this.partnerToken.address)
      expect((await this.chef.pendingTokens(0, this.bob.address)).bonusTokenSymbol).to.equal("SUSHI")
      ...
      // Bob should have:
      //   - 1500 + 1*50 + 950 + 1*50 = 2550 (+50) RipToken
      //   - 120 PartnerToken
      expect(await this.Rip.balanceOf(this.bob.address)).to.be.within(2550, 2600)
      expect(await this.partnerToken.balanceOf(this.bob.address)).to.equal(120)
    })

    it("should give out Rips only after farming time", async function () {
      const startTime = (await latest()).add(60)
      this.chef = await this.MCV2.deploy(
        this.Rip.address,
        this.dev.address,
      ...
      expect(await this.Rip.balanceOf(this.bob.address)).to.be.within(500, 550)
      ...
      // At this point:
      //   Bob should have:
      //     - 500 + 5*50 = 750 (+50) RipToken
      //     - 240 + 2*40 = 320 PartnerToken
      //   Dev should have: 15*20 = 300 (+20)
      //   Treasury should have: 15*20 = 300 (+20)
      //   Investor should have: 15*10 = 150 (+10)
      expect(await this.Rip.balanceOf(this.bob.address)).to.be.within(750, 800)
      expect(await this.partnerToken.balanceOf(this.bob.address)).to.equal("320")
      expect(await this.Rip.balanceOf(this.dev.address)).to.be.within(300, 320)
      expect(await this.Rip.balanceOf(this.treasury.address)).to.be.within(300, 320)
      expect(await this.Rip.balanceOf(this.investor.address)).to.be.within(150, 160)
      expect(await this.Rip.totalSupply()).to.be.within(1500, 1600)
    })

    it("should not distribute Rips if no one deposit", async function () {
      const startTime = (await latest()).add(60)
      this.chef = await this.MCV2.deploy(
        this.Rip.address,
        this.dev.address,
      ...
      expect(await this.Rip.totalSupply()).to.equal("0")
      ...
      expect(await this.Rip.balanceOf(this.bob.address)).to.equal("0")
      expect(await this.partnerToken.balanceOf(this.bob.address)).to.equal("0")
      expect(await this.Rip.balanceOf(this.dev.address)).to.equal("0")
      ...
      // At this point:
      //   Bob should have:
      //     - 12*50 = 600 (+50) RipToken
      //     - 3*40 = 120 PartnerToken
      //  Dev should have:
      //     - 12*20 = 240 (+20) RipToken
      //  Treasury should have:
      //     - 12*20 = 240 (+20) RipToken
      //  Investor should have:
      //     - 12*10 = 120 (+10) RipToken
      expect(await this.Rip.totalSupply()).to.be.within(1200, 1300)
      expect(await this.Rip.balanceOf(this.bob.address)).to.be.within(600, 650)
      expect(await this.Rip.balanceOf(this.dev.address)).to.be.within(240, 260)
      expect(await this.Rip.balanceOf(this.treasury.address)).to.be.within(240, 260)
      expect(await this.Rip.balanceOf(this.investor.address)).to.be.within(120, 130)
      expect(await this.partnerToken.balanceOf(this.bob.address)).to.equal(120)
    })

    it("should distribute Rips properly for each staker", async function () {
      const startTime = (await latest()).add(60)
      this.chef = await this.MCV2.deploy(
        this.Rip.address,
        this.dev.address,
      ...
      //   Alice should have:
      //      - 4*50 + 4*50*1/3 + 2*50*1/6 = 283 (+50) RipToken
      ...
      expect(await this.Rip.totalSupply()).to.be.within(1000, 1100)
      // Because LP rewards are divided among participants and rounded down, we account
      // for rounding errors with an offset
      expect(await this.Rip.balanceOf(this.alice.address)).to.be.within(283 - this.tokenOffset, 333 + this.tokenOffset)
      expect(await this.partnerToken.balanceOf(this.alice.address)).to.be.within(120 - this.tokenOffset, 120 + this.tokenOffset)

      expect(await this.Rip.balanceOf(this.bob.address)).to.equal("0")
      expect(await this.partnerToken.balanceOf(this.bob.address)).to.equal("0")

      expect(await this.Rip.balanceOf(this.carol.address)).to.equal("0")
      expect(await this.partnerToken.balanceOf(this.carol.address)).to.equal("0")

      expect(await this.Rip.balanceOf(this.dev.address)).to.be.within(200 - this.tokenOffset, 220 + this.tokenOffset)
      expect(await this.Rip.balanceOf(this.treasury.address)).to.be.within(200 - this.tokenOffset, 220 + this.tokenOffset)
      expect(await this.Rip.balanceOf(this.investor.address)).to.be.within(100 - this.tokenOffset, 110 + this.tokenOffset)
      expect(await this.Rip.balanceOf(this.chef.address)).to.be.within(217 - this.tokenOffset, 317 + this.tokenOffset)
      // Bob withdraws 5 LPs at t+30. At this point:
      //   Bob should have:
      //     - 4*50*2/3 + 2*50*2/6 + 10*50*2/7 = 309 (+50) RipToken
      ...
      expect(await this.Rip.totalSupply()).to.be.within(2000, 2100)
      ...
      expect(await this.Rip.balanceOf(this.bob.address)).to.be.within(309 - this.tokenOffset, 359 + this.tokenOffset)
      expect(await this.partnerToken.balanceOf(this.bob.address)).to.be.within(101 - this.tokenOffset, 101 + this.tokenOffset)

      expect(await this.Rip.balanceOf(this.carol.address)).to.equal("0")
      expect(await this.partnerToken.balanceOf(this.carol.address)).to.equal("0")

      expect(await this.Rip.balanceOf(this.dev.address)).to.be.within(400 - this.tokenOffset, 420 + this.tokenOffset)
      expect(await this.Rip.balanceOf(this.treasury.address)).to.be.within(400 - this.tokenOffset, 420 + this.tokenOffset)
      expect(await this.Rip.balanceOf(this.investor.address)).to.be.within(200 - this.tokenOffset, 210 + this.tokenOffset)
      expect(await this.Rip.balanceOf(this.chef.address)).to.be.within(408 - this.tokenOffset, 508 + this.tokenOffset)
      ...
      expect(await this.Rip.totalSupply()).to.be.within(5000, 5100)
      // Alice should have:
      //  - 283 + 10*50*2/7 + 10*50*20/65 = 579 (+50) RipToken
      //  - 120 + 2*40*2/7 + 2*40*20/65 = 167 PartnerToken
      expect(await this.Rip.balanceOf(this.alice.address)).to.be.within(579 - this.tokenOffset, 629 + this.tokenOffset)
      expect(await this.partnerToken.balanceOf(this.alice.address)).to.be.within(167 - this.tokenOffset, 167 + this.tokenOffset)
      // Bob should have:
      //  - 309 + 10*50*15/65 + 10*50*15/45 = 591 (+50) RipToken
      //  - 102 + 2*40*15/65 + 2*40*15/45 = 147 PartnerToken
      expect(await this.Rip.balanceOf(this.bob.address)).to.be.within(591 - this.tokenOffset, 641 + this.tokenOffset)
      expect(await this.partnerToken.balanceOf(this.bob.address)).to.be.within(147 - this.tokenOffset, 147 + this.tokenOffset)
      // Carol should have:
      //  - 2*50*3/6 + 10*50*3/7 + 10*50*30/65 + 10*50*30/45 + 10*50 = 1445 (+50) RipToken
      //  - 2*40*1/2 + 2*40*3/7 + 2*40*30/65 + 2*40*30/45 + 2*40 = 244 PartnerToken
      expect(await this.Rip.balanceOf(this.carol.address)).to.be.within(1328 - this.tokenOffset, 1378 + this.tokenOffset)
      ...
      expect(await this.Rip.balanceOf(this.dev.address)).to.be.within(1000 - this.tokenOffset, 1020 + this.tokenOffset)
      expect(await this.Rip.balanceOf(this.treasury.address)).to.be.within(1000 - this.tokenOffset, 1020 + this.tokenOffset)
      expect(await this.Rip.balanceOf(this.investor.address)).to.be.within(500 - this.tokenOffset, 510 + this.tokenOffset)
      // MasterChefRip should have nothing
      expect(await this.Rip.balanceOf(this.chef.address)).to.be.within(0, 0 + this.tokenOffset)
    ...
    it("should give proper Rips allocation to each pool", async function () {
      const startTime = (await latest()).add(60)
      this.chef = await this.MCV2.deploy(
        this.Rip.address,
        this.dev.address,
      ...
      //   - 10*50 = 500 (+50) RipToken
      //   - 2*40 = 80 PartnerToken
      expect((await this.chef.pendingTokens(0, this.alice.address)).pendingRip).to.be.within(500 - this.tokenOffset, 550 + this.tokenOffset)
      ...
      // Alice's pending reward should be:
      //   - 500 + 5*1/3*50 = 583 (+50) RipToken
      //   - 80 + 2*40 = 160 PartnerToken
      expect((await this.chef.pendingTokens(0, this.alice.address)).pendingRip).to.be.within(583 - this.tokenOffset, 633 + this.tokenOffset)
      expect(await this.rewarder.pendingTokens(this.alice.address)).to.equal(160)

      // At this point:
      //   Alice's pending reward should be:
      //     - 583 + 5*1/3*50 = 666 (+50) RipToken
      //     - 160 + 1*40 = 200 PartnerToken
      // Bob's pending reward should be:
      //     - 5*2/3*50 = 166 (+50) RipToken
      //     - 0 PartnerToken
      await advanceTimeAndBlock(5) // t+30, b=27
      expect((await this.chef.pendingTokens(0, this.alice.address)).pendingRip).to.be.within(666 - this.tokenOffset, 716 + this.tokenOffset)
      expect(await this.rewarder.pendingTokens(this.alice.address)).to.equal(200)

      expect((await this.chef.pendingTokens(1, this.bob.address)).pendingRip).to.be.within(166 - this.tokenOffset, 216 + this.tokenOffset)
      ...
      expect((await this.chef.pendingTokens(1, this.alice.address)).pendingRip).to.equal("0")
      expect((await this.chef.pendingTokens(0, this.bob.address)).pendingRip).to.equal("0")
      ...
      // Alice should have:
      //   - 666 + 1*1/3*50 = 682 (+50) RipToken
      //   - 200 + 1*40 = 240 PartnerToken
      expect(await this.Rip.balanceOf(this.alice.address)).to.be.within(682 - this.tokenOffset, 732 + this.tokenOffset)
      ...
      // Bob should have:
      //   - 166 + 2*2/3*50 = 232 (+50) RipToken
      //   - 0 PartnerToken
      expect(await this.Rip.balanceOf(this.bob.address)).to.be.within(232 - this.tokenOffset, 282 + this.tokenOffset)
      expect(await this.rewarder.pendingTokens(this.bob.address)).to.equal(0)
    })

    it("should give proper Rips after updating emission rate", async function () {
      const startTime = (await latest()).add(60)
      this.chef = await this.MCV2.deploy(
        this.Rip.address,
        this.dev.address,
      ...
      //   - 100*50 = 5000 (+50) RipToken
      //   - 1*40 = 40 PartnerToken
      await advanceTimeAndBlock(100) // t+110, b=22
      expect((await this.chef.pendingTokens(0, this.alice.address)).pendingRip).to.be.within(5000, 5050)
      expect(await this.rewarder.pendingTokens(this.alice.address)).to.equal(40)
      // Lower Rip emission rate to 40 Rip per sec
      await this.chef.updateEmissionRate(40) // t+111, b=23
      // At t+115, Alice should have:
      //   - 5000 + 1*100*0.5 + 4*40*0.5 = 5130 (+50) RipToken
      //   - 40 + 2*40 = 120 PartnerToken
      await advanceTimeAndBlock(4) // t+115, b=24
      expect((await this.chef.pendingTokens(0, this.alice.address)).pendingRip).to.be.within(5130, 5180)
      ...
      //   - 5130 + 1*40*0.5 + 20*40*0.5 = 5550 (+50) RipToken
      ...
      expect((await this.chef.pendingTokens(0, this.alice.address)).pendingRip).to.be.within(5550, 5600)
  ...
  context("With ERC/LP token added to the field and using SimpleRewarderPerSec", function () {
    ...
    it("should check rewarder's arguments are contracts", async function () {
      ...
      ).to.be.revertedWith("constructor: MasterChefRip must be a valid contract")
    })
    ...
    it("should reward partner token accurately after rewarder runs out of tokens and is topped up again", async function () {
      ...
      await this.Rip.transferOwnership(this.chef.address) // t-56
    ...
    it("should only allow MasterChefRipV2 to call onRipReward", async function () {
      ...
      await expect(this.rewarder.onRipReward(this.bob.address, "100")).to.be.revertedWith("onlyMCJ: only MasterChefRip can call this function") // t-10
    ...
    it("should distribute Rips properly for each staker", async function () {
      ...
      // Carol should have:
      //  - 2*50*3/6 + 10*50*3/7 + 10*50*30/65 + 10*50*30/45 + 10*50 = 1328 (+50) RipToken
    ...
    it("should give proper Rips after updating emission rate", async function () {
      ...
      // At b=35, Alice should have:
      //   - 5130 + 21*40*0.5 = 5550 (+50) RipToken
  ...
  context("With ERC/LP token added to the field and using MasterChefRewarderPerBlock", function () {
    ...
    it("should only allow MasterChefRipV2 to call onRipReward", async function () {
      ...
      await this.Rip.transferOwnership(this.chef.address) // t-53, b=20
      ...
      await expect(this.rewarder.onRipReward(this.bob.address, "100")).to.be.revertedWith("onlyMCV2: only MasterChef V2 can call this function") // t-10, b=25
    ...
    it("should allow allocation point to be set mid farming", async function () {
      ...
      await this.Rip.transferOwnership(this.chef.address) // t-52, b=21
      ...
      // Bob should have:
      //   - 500 + 20*50 = 1500 (+50) RipToken
      //   - 79 + 3*40*1/4 + 4*40*1/2 = 189 PartnerToken
      expect(await this.Rip.balanceOf(this.bob.address)).to.be.within(1500, 1550)
    ...
    it("should distribute Rips properly for each staker", async function () {
      ...
      expect(await this.Rip.balanceOf(this.treasury.address)).to.be.within(1000 - this.tokenOffset, 1020 + this.tokenOffset)
      expect(await this.Rip.balanceOf(this.investor.address)).to.be.within(500 - this.tokenOffset, 510 + this.tokenOffset)
      // MasterChefRip and PartnerChef should have nothing
      expect(await this.Rip.balanceOf(this.chef.address)).to.be.within(0, 0 + this.tokenOffset)
  ...
  context("With ERC/LP token added to the field and using MasterChefRewarderPerSec", function () {
    ...
    it("should only allow MasterChefRipV2 to call onRipReward", async function () {
      ...
      await this.Rip.transferOwnership(this.chef.address) // t-53
      ...
      await expect(this.rewarder.onRipReward(this.bob.address, "100")).to.be.revertedWith("onlyMCV2: only MasterChef V2 can call this function") // t-10
      await this.chef.connect(this.bob).deposit(0, "0") // t-9
      // Bob should have:
      //   - 0 RipToken
    ...
    it("should allow allocation point to be set mid farming", async function () {
      ...
      await this.Rip.transferOwnership(this.chef.address) // t-52
