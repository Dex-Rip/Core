...
// Test values in order to be sure RipMakerV3 converts as much as RipMakerV1
const barBalanceRipAVAX = "67845624860978841228702792"
const barBalanceDAIUSDC = "67845624656456566165626771"

describe("RipMakerV3", function () {
  before(async function () {
    // ABIs
    this.RipMakerV3CF = await ethers.getContractFactory("RipMakerV3")
    ...
    // Pairs
    this.RipAvax = await this.PairCF.attach(RipAVAX_ADDRESS, this.dev)
    ...
    this.RipUsdt = await this.PairCF.attach(RipUSDT_ADDRESS, this.dev)
  ...
  beforeEach(async function () {
    ...
    // We redeploy RipMakerV3 for each tests too
    this.RipMakerV3 = await this.RipMakerV3CF.deploy(FACTORY_ADDRESS, BAR_ADDRESS, Rip_ADDRESS, WAVAX_ADDRESS)
    await this.RipMakerV3.deployed()
  })

  describe("setBridge", function () {
    it("does not allow to set bridge for Rip", async function () {
      await expect(this.RipMakerV3.setBridge(this.Rip.address, this.wavax.address)).to.be.revertedWith("RipMakerV3: Invalid bridge")
    })

    it("does not allow to set bridge for WAVAX", async function () {
      await expect(this.RipMakerV3.setBridge(this.wavax.address, this.Rip.address)).to.be.revertedWith("RipMakerV3: Invalid bridge")
    })

    it("does not allow to set bridge to itself", async function () {
      await expect(this.RipMakerV3.setBridge(this.dai.address, this.dai.address)).to.be.revertedWith("RipMakerV3: Invalid bridge")
    })

    it("emits correct event on bridge", async function () {
      await expect(this.RipMakerV3.setBridge(this.dai.address, this.Rip.address))
        .to.emit(this.RipMakerV3, "LogBridgeSet")
  ...
  describe("convert", function () {
    it("should convert Rip - AVAX", async function () {
      ...
      await this.RipAvax.transfer(this.RipMakerV3.address, await this.RipAvax.balanceOf(this.dev.address))
      await this.RipMakerV3.convert(this.Rip.address, this.wavax.address)
      expect(await this.Rip.balanceOf(this.RipMakerV3.address)).to.equal(0)
      expect(await this.RipAvax.balanceOf(this.RipMakerV3.address)).to.equal(0)
      expect(await this.Rip.balanceOf(BAR_ADDRESS)).to.equal(barBalanceRipAVAX)
    })

    it("should convert Rip - AVAX using RipMakerV1 to make sure V3 converts as much as V1", async function () {
      ...
      await this.zap.zapIn(this.RipAvax.address, { value: "2000000000000000000" })
      await this.RipAvax.transfer(this.RipMaker.address, await this.RipAvax.balanceOf(this.dev.address))
    ...
    it("should convert USDC - AVAX", async function () {
      ...
      await this.usdcAvax.transfer(this.RipMakerV3.address, await this.usdcAvax.balanceOf(this.dev.address))
      await this.RipMakerV3.convert(this.usdc.address, this.wavax.address)
      expect(await this.Rip.balanceOf(this.RipMakerV3.address)).to.equal(0)
      expect(await this.usdcAvax.balanceOf(this.RipMakerV3.address)).to.equal(0)
    ...
    it("should convert LINK - AVAX", async function () {
      ...
      await this.linkAvax.transfer(this.RipMakerV3.address, await this.linkAvax.balanceOf(this.dev.address))
      await this.RipMakerV3.convert(this.link.address, this.wavax.address)
      expect(await this.Rip.balanceOf(this.RipMakerV3.address)).to.equal(0)
      expect(await this.linkAvax.balanceOf(this.RipMakerV3.address)).to.equal(0)
    ...
    it("reverts if convert is called by non auth", async function () {
      await this.zap.zapIn(this.RipAvax.address, { value: "2000000000000000000" })
      await this.RipAvax.transfer(this.RipMakerV3.address, await this.RipAvax.balanceOf(this.dev.address))
      await expect(this.RipMakerV3.connect(this.alice).convert(this.Rip.address, this.wavax.address)).to.be.revertedWith("RipMakerV3: FORBIDDEN")
    })

    it("should convert USDT - Rip", async function () {
      await this.zap.zapIn(this.RipUsdt.address, { value: "2000000000000000000" })
      await this.RipUsdt.transfer(this.RipMakerV3.address, await this.RipUsdt.balanceOf(this.dev.address))
      await this.RipMakerV3.convert(this.usdt.address, this.Rip.address)
      expect(await this.Rip.balanceOf(this.RipMakerV3.address)).to.equal(0)
      expect(await this.RipUsdt.balanceOf(this.RipMakerV3.address)).to.equal(0)
    ...
    it("should convert using standard AVAX path", async function () {
      ...
      await this.daiAvax.transfer(this.RipMakerV3.address, await this.daiAvax.balanceOf(this.dev.address))
      await this.RipMakerV3.convert(this.dai.address, this.wavax.address)
      expect(await this.Rip.balanceOf(this.RipMakerV3.address)).to.equal(0)
      expect(await this.daiAvax.balanceOf(this.RipMakerV3.address)).to.equal(0)
    ...
    it("converts LINK/USDC using more complex path", async function () {
      ...
      await this.linkUsdc.transfer(this.RipMakerV3.address, await this.linkUsdc.balanceOf(this.dev.address))
      await this.RipMakerV3.setBridge(this.usdt.address, this.Rip.address)
      await this.RipMakerV3.setBridge(this.usdc.address, this.usdt.address)
      await this.RipMakerV3.convert(this.link.address, this.usdc.address)
      expect(await this.Rip.balanceOf(this.RipMakerV3.address)).to.equal(0)
      expect(await this.linkUsdc.balanceOf(this.RipMakerV3.address)).to.equal(0)
    ...
    it("converts DAI/USDC using more complex path", async function () {
      ...
      await this.usdcDai.transfer(this.RipMakerV3.address, await this.usdcDai.balanceOf(this.dev.address))
      await this.RipMakerV3.setBridge(this.usdc.address, this.Rip.address)
      await this.RipMakerV3.setBridge(this.dai.address, this.usdc.address)
      await this.RipMakerV3.convert(this.dai.address, this.usdc.address)
      expect(await this.Rip.balanceOf(this.RipMakerV3.address)).to.equal(0)
      expect(await this.usdcDai.balanceOf(this.RipMakerV3.address)).to.equal(0)
      expect(await this.Rip.balanceOf(BAR_ADDRESS)).to.equal(barBalanceDAIUSDC)
    })

    it("converts DAI/USDC using more complex path and RipMakerV1 to make sure V3 converts as much as V1", async function () {
      this.RipMaker = await this.RipMakerCF.deploy(FACTORY_ADDRESS, BAR_ADDRESS, Rip_ADDRESS, WAVAX_ADDRESS)
      await this.RipMaker.deployed()

      await this.zap.zapIn(this.usdcDai.address, { value: "2000000000000000000" })
      await this.usdcDai.transfer(this.RipMaker.address, await this.usdcDai.balanceOf(this.dev.address))
    ...
    it("should convert reflect tokens TRACTOR/AVAX", async function () {
      ...
      this.router.addLiquidityAVAX(this.tractor.address, balance, "0", "0", this.RipMakerV3.address, "11111111111111111", {
        value: "1000000000000000000",
      })

      await this.RipMakerV3.convert(this.tractor.address, this.wavax.address)
    ...
    it("reverts if it loops back", async function () {
      await this.zap.zapIn(this.usdcDai.address, { value: "2000000000000000000" })
      await this.usdcDai.transfer(this.RipMakerV3.address, await this.wbtcUsdc.balanceOf(this.dev.address))
      await this.RipMakerV3.setBridge(this.wbtc.address, this.usdc.address)
      await this.RipMakerV3.setBridge(this.usdc.address, this.wbtc.address)
      await expect(this.RipMakerV3.convert(this.dai.address, this.usdc.address)).to.be.reverted
    })

    it("reverts if caller is not EOA", async function () {
      const exploiterCF = await ethers.getContractFactory("RipMakerExploitMock")
      const exploiter = await exploiterCF.deploy(this.RipMakerV3.address)
      await exploiter.deployed()

      await this.zap.zapIn(this.RipAvax.address, { value: "2000000000000000000" })
      await this.RipAvax.transfer(this.RipMakerV3.address, await this.wbtcUsdc.balanceOf(this.dev.address))
      await expect(exploiter.convert(this.Rip.address, this.wavax.address)).to.be.revertedWith("RipMakerV3: must use EOA")
    })

    it("reverts if pair does not exist", async function () {
      await expect(this.RipMakerV3.convert(this.Rip.address, this.RipAvax.address)).to.be.revertedWith("RipMakerV3: Invalid pair")
    })
  })
  ...
  describe("convertMultiple", function () {
    it("should allow to convert multiple", async function () {
      ...
      await this.daiAvax.transfer(this.RipMakerV3.address, await this.daiAvax.balanceOf(this.dev.address))
      await this.RipAvax.transfer(this.RipMakerV3.address, await this.RipAvax.balanceOf(this.dev.address))
      await this.RipMakerV3.convertMultiple([this.dai.address, this.Rip.address], [this.wavax.address, this.wavax.address])
      expect(await this.Rip.balanceOf(this.RipMakerV3.address)).to.equal(0)
      expect(await this.daiAvax.balanceOf(this.RipMakerV3.address)).to.equal(0)
  ...
  describe("devCut", function () {
    it("should redirect 50% of Rip to dev address", async function () {
      await this.RipMakerV3.setDevAddr(this.dev.address)
      await this.RipMakerV3.setDevCut("5000")

      this.wavaxERC20 = await this.ERC20CF.attach(WAVAX_ADDRESS)

      const barBalance = await this.Rip.balanceOf(BAR_ADDRESS)
      ...
      expect(await this.Rip.balanceOf(this.RipMakerV3.address)).to.equal(0)
      expect(await this.usdcAvax.balanceOf(this.RipMakerV3.address)).to.equal(0)
      expect((await this.Rip.balanceOf(BAR_ADDRESS)) - barBalance).to.be.greaterThan(0)
