...
contract MoneyMaker is Ownable {
    ...
    /// @notice Constructor
    /// @param _factory The address of RipFactory
    /// @param _bar The address of RipBar
    ...
    function _swap(
        ...
        amountOut = RipLibrary.getAmountOut(amountInput, reserveInput, reserveOutput);

        {
            ...
            require(
                RipLibrary.getAmountOut(amountOut, reserveOutput, reserveInput) >=
    ...
    /// @param slippage The accepted slippage, in basis points aka parts per 10,000 so 5000 is 50%
    /// @return amountOut The amount of `toToken` sent to RipBar
    function _toToken(
        address token,
        uint256 amountIn,
        uint256 slippage
