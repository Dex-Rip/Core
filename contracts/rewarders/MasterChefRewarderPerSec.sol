...
interface IMasterChefRipV2 {
    ...
    struct PoolInfo {
        ...
        uint256 accRipPerShare; // Accumulated SUSHI per share, times 1e12. See below.
    }
...
/**
 * This is a sample contract to be used in the MasterChefRipV2 contract for partners to reward
 * stakers with their native token alongside Rip.
 *
...
contract MasterChefRewarderPerSec is IRewarder, Ownable {
    ...
    IMasterChefRipV2 public immutable MCV2;
    ...
    /// `accTokenPerShare` Amount of Rip each LP token is worth.
    /// `lastRewardTimestamp` The last time Rip was rewarded to the poolInfo.
    struct PoolInfo {
    ...
    constructor(
        ...
        require(Address.isContract(address(_MCV2)), "constructor: MasterChefRipV2 must be a valid contract");
    ...
    /// @notice Function called by MasterChefRipV2 whenever staker claims Rip harvest. Allows staker to also receive a 2nd reward token.
    /// @param _user Address of user
    /// @param _lpAmount Number of LP tokens the user has
    function onRipReward(address _user, uint256 _lpAmount) external override onlyMCV2 {
        updatePool();
