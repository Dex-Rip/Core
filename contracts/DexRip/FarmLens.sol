...
import "./interfaces/IRipERC20.sol";
import "./interfaces/IRipPair.sol";
...
contract FarmLens is BoringOwnable {
    ...
    IRipFactory public RipFactory; // IRipFactory(0x9Ad6C38BE94206cA50bb0d90783181662f0Cfa10);
    ...
    constructor(
        ...
        wavaxDai = wavaxDai_;
        RipFactory = IRipFactory(RipFactory_);
        chefv2 = chefv2_;
        chefv3 = chefv3_;
    }
    ...
    function getAvaxPrice() public view returns (uint256) {
        uint256 priceFromWavaxUsdt = _getAvaxPrice(IRipPair(wavaxUsdt)); // 18
        uint256 priceFromWavaxUsdc = _getAvaxPrice(IRipPair(wavaxUsdc)); // 18
        uint256 priceFromWavaxDai = _getAvaxPrice(IRipPair(wavaxDai)); // 18
    ...
    function _getAvaxPrice(IRipPair pair) private view returns (uint256) {
    ...
    function getPriceInAvax(address tokenAddress) public view returns (uint256) {
        ...
        IRipPair pair = IRipPair(RipFactory.getPair(tokenAddress, wavax));
    ...
    function _tokenDecimalsMultiplier(address tokenAddress) private pure returns (uint256) {
        uint256 decimalsNeeded = 18 - IRipERC20(tokenAddress).decimals();
    ...
    function getReserveUsd(IRipPair pair) public view returns (uint256) {
        address token0Address = pair.token0();
        address token1Address = pair.token1();
        ...
        returns (FarmPair[] memory)
    {
        ...
        for (uint256 i = 0; i < whitelistLength; i++) {
            ...
            IRipPair lpToken = IRipPair(address(pool.lpToken));
            ...
            farmPairs[i].token0Symbol = IRipERC20(token0Address).symbol();
            farmPairs[i].token1Symbol = IRipERC20(token1Address).symbol();
            ...
            farmPairs[i].chefRipPerSec = chef.RipPerSec();
        }
    ...
    struct AllFarmData {
        ...
        uint256 RipPerSecChefV2;
        uint256 RipPerSecChefV3;
        ...
        returns (AllFarmData memory)
    {
        ...
        allFarmData.RipPriceUsd = getPriceInUsd(Rip);

        allFarmData.totalAllocChefV2 = IMasterChef(chefv2).totalAllocPoint();
        allFarmData.RipPerSecChefV2 = IMasterChef(chefv2).RipPerSec();

        allFarmData.totalAllocChefV3 = IMasterChef(chefv3).totalAllocPoint();
        allFarmData.RipPerSecChefV3 = IMasterChef(chefv3).RipPerSec();
