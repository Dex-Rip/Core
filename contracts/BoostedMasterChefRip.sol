...
import "./interfaces/IMasterChefRip.sol";
import "./interfaces/IRewarder.sol";
import "./libraries/BoringRipERC20.sol";

/// @notice The (older) MasterChefRipV2 contract gives out a constant number of Rip
/// tokens per block.  It is the only address with minting rights for Rip.  The idea
/// for this BoostedMasterChefRip (BMCJ) contract is therefore to be the owner of a
/// dummy token that is deposited into the MasterChefRipV2 (MCJV2) contract.  The
...
/// amount of veRip held by the user over the total veRip amount held by all pool
/// participants. Total liquidity is the sum of all boosted liquidity.
contract BoostedMasterChefRip is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /// `rewardDebt` The amount of Rip entitled to the user
    /// `veRipBalance` the users balance of veRip token. This needs
    /// to be stored so we have a prior value when updating the `PoolInfo`
    struct UserInfo {
        uint256 amount;
        uint256 rewardDebt;
        uint256 veRipBalance;
    }

    /// @notice Info of each BMCJ pool
    /// `allocPoint` The amount of allocation points assigned to the pool
    /// Also known as the amount of Rip to distribute per block
    struct PoolInfo {
        IERC20 lpToken;
        uint256 accRipPerShare;
        uint256 lastRewardTimestamp;
        uint256 allocPoint;
        IRewarder rewarder;
        // The sum of all veRip held by users participating in this farm
    ...
    IMasterChefRip public MASTER_CHEF_V2;
    /// @notice Address of Rip contract
    IERC20 public Rip;
    /// @notice Address of veRip contract
    IERC20 public VERip;
    ...
    /// @dev Amount of claimable Rip the user has, this is required as we
    ...
    mapping(uint256 => mapping(address => uint256)) public claimableRip;
    ...
    event UpdatePool(uint256 indexed pid, uint256 lastRewardTimestamp, uint256 lpSupply, uint256 accRipPerShare);
    event Harvest(address indexed user, uint256 indexed pid, uint256 amount);
    event EmergencyWithdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event Init(uint256 amount);

    /// @param _MASTER_CHEF_V2 The MCJV2 contract address
    /// @param _Rip The Rip token contract address
    /// @param _veRip The veRip token contract address
    /// @param _MASTER_PID The pool ID of the dummy token on the base MCJV2 contract
    function initialize(
        IMasterChefRip _MASTER_CHEF_V2,
        IERC20 _Rip,
        IERC20 _veRip,
        uint256 _MASTER_PID
    ) public initializer {
    ...
    /// holds the minting rights for Rip.  Any balance of transaction sender in `_dummyToken` is transferred.
    ...
    function init(IERC20 _dummyToken) external onlyOwner {
        require(
            ...
            "BoostedMasterChefRip: Already has a balance of dummy token"
        );
        uint256 balance = _dummyToken.balanceOf(msg.sender);
        require(balance != 0, "BoostedMasterChefRip: Balance must exceed 0");
    ...
    function add(
        ...
        require(!lpTokens.contains(address(_lpToken)), "BoostedMasterChefRip: LP already added");
        require(poolInfo.length <= 50, "BoostedMasterChefRip: Too many pools");
        // Sanity check to ensure _lpToken is an ERC20 token
        _lpToken.balanceOf(address(this));
        // Sanity check if we add a rewarder
        if (address(_rewarder) != address(0)) {
            _rewarder.onRipReward(address(0), 0);
        }
    ...
    /// @notice Update the given pool's Rip allocation point and `IRewarder` contract. Can only be called by the owner.
    ...
    /// @notice View function to see pending Rip on frontend
    /// @param _pid The index of the pool. See `poolInfo`
    /// @param _user Address of user
    /// @return pendingRip Rip reward for a given user.
        ...
        )
    {
        ...
        uint256 accRipPerShare = pool.accRipPerShare;
        ...
        if (block.timestamp > pool.lastRewardTimestamp && pool.totalBoostedAmount != 0) {
            ...
            uint256 RipReward = secondsElapsed.mul(RipPerSec()).mul(pool.allocPoint).div(totalAllocPoint);
            accRipPerShare = accRipPerShare.add(RipReward.mul(ACC_TOKEN_PRECISION).div(pool.totalBoostedAmount));
        }

        uint256 userLiquidity = getBoostedLiquidity(_pid, _user);
        pendingRip = userLiquidity.mul(accRipPerShare).div(ACC_TOKEN_PRECISION).sub(user.rewardDebt).add(
            claimableRip[_pid][_user]
        );
    ...
    /// @notice Calculates and returns the `amount` of Rip per second
    /// @return amount The amount of Rip emitted per second
    function RipPerSec() public view returns (uint256 amount) {
        uint256 total = 1000;
        ...
        uint256 lpShare = MASTER_CHEF_V2.RipPerSec().mul(lpPercent).div(total);
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (block.timestamp > pool.lastRewardTimestamp) {
            ...
            if (lpSupply != 0) {
                uint256 secondsElapsed = block.timestamp.sub(pool.lastRewardTimestamp);
                uint256 RipReward = secondsElapsed.mul(RipPerSec()).mul(pool.allocPoint).div(totalAllocPoint);
                pool.accRipPerShare = pool.accRipPerShare.add(
                    RipReward.mul(ACC_TOKEN_PRECISION).div(pool.totalBoostedAmount)
                );
            }
            pool.lastRewardTimestamp = block.timestamp;
            poolInfo[_pid] = pool;
            emit UpdatePool(_pid, pool.lastRewardTimestamp, lpSupply, pool.accRipPerShare);
        }
    }

    /// @notice Deposit LP tokens to BMCJ for Rip allocation
    /// @param _pid The index of the pool. See `poolInfo`
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount != 0) {
            ...
            uint256 pending = boostedLiquidity
                .mul(pool.accRipPerShare)
                .div(ACC_TOKEN_PRECISION)
                .sub(user.rewardDebt)
                .add(claimableRip[_pid][msg.sender]);
            claimableRip[_pid][msg.sender] = 0;
            Rip.safeTransfer(msg.sender, pending);
        ...
        // Update the `pool.totalVeRip`
        uint256 veRipBalance = VERip.balanceOf(msg.sender);
        if (veRipBalance != 0) {
            pool.totalVeRip = pool.totalVeRip.add(veRipBalance).sub(user.veRipBalance);
        }
        user.veRipBalance = veRipBalance;
        ...
        user.rewardDebt = boostedLiquidityAfter.mul(pool.accRipPerShare).div(ACC_TOKEN_PRECISION);

        // Interactions
        IRewarder _rewarder = pool.rewarder;
        if (address(_rewarder) != address(0)) {
            _rewarder.onRipReward(msg.sender, user.amount);
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        uint256 boostedLiquidityAfter = getBoostedLiquidity(_pid, msg.sender);
        // Update the `pool.totalBoostedAmount` and `totalVeRip` if needed
        if (user.amount == 0) {
            pool.totalVeRip = pool.totalVeRip.sub(user.veRipBalance);
        }
    ...
    /// a veRip token operation. This function needs to be called
    /// by the veRip contract after every event.
    /// @param _user The users address we are updating
    /// @param _newUserBalance The new balance of the users veRip
    function updateBoost(address _user, uint256 _newUserBalance) external {
        require(msg.sender == address(VERip), "BoostedMasterChefRip: Caller not veRip");
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            uint256 pending = oldBoostedLiquidity.mul(pool.accRipPerShare).div(ACC_TOKEN_PRECISION).sub(
                user.rewardDebt
            );

            // Increase claimableRip
            claimableRip[pid][_user] = claimableRip[pid][_user].add(pending);

            // Update users veRipBalance
            uint256 _oldBalance = user.veRipBalance;
            user.veRipBalance = _newUserBalance;

            // Update the pool total veRip
            pool.totalVeRip = pool.totalVeRip.add(_newUserBalance).sub(_oldBalance);
            ...
            user.rewardDebt = newBoostedLiquidity.mul(pool.accRipPerShare).div(ACC_TOKEN_PRECISION);
        }
    }
    ...
    function getUserBoost(uint256 _pid, address _user) public view returns (uint256 userBoost) {
        ...
        UserInfo memory user = userInfo[_pid][_user];
        userBoost = 1000;
        if (user.veRipBalance != 0 && pool.totalVeRip != 0) {
            userBoost = userBoost.add(user.veRipBalance.mul(maxBoostFactor).div(pool.totalVeRip));
        }
    }

    /// @notice Harvests Rip from `MASTER_CHEF_V2` MCJV2 and pool `MASTER_PID` to this BMCJ contract
    ...
    function emergencyWithdraw(uint256 _pid) external nonReentrant {
        ...
        IRewarder _rewarder = pool.rewarder;
        if (address(_rewarder) != address(0)) {
            _rewarder.onRipReward(msg.sender, 0);
        }
