...
contract MasterChefRipV2 is Ownable {
        ...
        )
    {
        ...
        if (block.timestamp > pool.lastRewardTimestamp && lpSupply != 0) {
            ...
            uint256 RipReward = multiplier.mul(RipPerSec).mul(pool.allocPoint).div(totalAllocPoint).mul(lpPercent).div(
                1000
            );
            accRipPerShare = accRipPerShare.add(RipReward.mul(1e12).div(lpSupply));
        }
        pendingRip = user.amount.mul(accRipPerShare).div(1e12).sub(user.rewardDebt);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = block.timestamp.sub(pool.lastRewardTimestamp);
        uint256 RipReward = multiplier.mul(RipPerSec).mul(pool.allocPoint).div(totalAllocPoint);
        uint256 lpPercent = 1000 - devPercent - treasuryPercent - investorPercent;
        ...
        pool.accRipPerShare = pool.accRipPerShare.add(RipReward.mul(1e12).div(lpSupply).mul(lpPercent).div(1000));
        pool.lastRewardTimestamp = block.timestamp;
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            // Harvest Rip
            uint256 pending = user.amount.mul(pool.accRipPerShare).div(1e12).sub(user.rewardDebt);
