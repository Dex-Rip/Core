...
/// @notice The (older) MasterChefRipV2 contract gives out a constant number of Rip tokens per block.
/// It is the only address with minting rights for Rip.
/// The idea for this MasterChefRipV3 (MCJV3) contract is therefore to be the owner of a dummy token
/// that is deposited into the MasterChefRipV2 (MCJV2) contract.
/// The allocation point for this pool on MCJV3 is the total allocation point for all pools that receive double incentives.
contract MasterChefRipV3 is Ownable, ReentrancyGuard {
    ...
    /// @param _MASTER_CHEF_V2 The RipSwap MCJV2 contract address.
    ...
    /// @notice Deposits a dummy token to `MASTER_CHEF_V2` MCJV2. This is required because MCJV2 holds the minting rights for Rip.
    ...
    function set(
        ...
        if (overwrite) {
            _rewarder.onRipReward(address(0), 0); // sanity check
        ...
        )
    {
        ...
        if (block.timestamp > pool.lastRewardTimestamp && lpSupply != 0) {
            ...
            accRipPerShare = accRipPerShare.add(RipReward.mul(ACC_TOKEN_PRECISION).div(lpSupply));
        }
        pendingRip = user.amount.mul(accRipPerShare).div(ACC_TOKEN_PRECISION).sub(user.rewardDebt);
    ...
    /// @notice Calculates and returns the `amount` of Rip per block.
    ...
    function updatePool(uint256 pid) public {
        ...
        if (block.timestamp > pool.lastRewardTimestamp) {
            ...
            if (lpSupply > 0) {
                uint256 secondsElapsed = block.timestamp.sub(pool.lastRewardTimestamp);
                uint256 RipReward = secondsElapsed.mul(RipPerSec()).mul(pool.allocPoint).div(totalAllocPoint);
                pool.accRipPerShare = pool.accRipPerShare.add((RipReward.mul(ACC_TOKEN_PRECISION).div(lpSupply)));
            }
            pool.lastRewardTimestamp = block.timestamp;
            poolInfo[pid] = pool;
            emit UpdatePool(pid, pool.lastRewardTimestamp, lpSupply, pool.accRipPerShare);
        }
    }

    /// @notice Deposit LP tokens to MCJV3 for Rip allocation.
    /// @param pid The index of the pool. See `poolInfo`.
    ...
    function deposit(uint256 pid, uint256 amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            // Harvest Rip
            uint256 pending = user.amount.mul(pool.accRipPerShare).div(ACC_TOKEN_PRECISION).sub(user.rewardDebt);
        ...
        user.rewardDebt = user.amount.mul(pool.accRipPerShare).div(ACC_TOKEN_PRECISION);
    ...
    /// @notice Harvests Rip from `MASTER_CHEF_V2` MCJV2 and pool `MASTER_PID` to this MCJV3 contract.
