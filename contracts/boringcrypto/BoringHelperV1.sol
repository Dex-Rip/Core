...
interface IMasterChef {
    ...
    function Rip() external view returns (address);

    function RipPerBlock() external view returns (uint256);
...
contract BoringHelperV1 is Ownable {
    ...
    address public maker; // IRipMaker(0xE11fc0B43ab98Eb91e9836129d1ee7c3Bc95df50);
    IERC20 public Rip; // IRipToken(0x6B3595068778DD592e39A122f4f5a5cF09C90fE2);
    IERC20 public WAVAX; // 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    IFactory public RipFactory; // IFactory(0xC0AEe478e3658e2610c5F7A4A2E1777cE9e4f2Ac);
    ...
    function getAVAXRate(IERC20 token) public view returns (uint256) {
        ...
        if (RipFactory != IFactory(0)) {
            pairRip = IPair(RipFactory.getPair(token, WAVAX));
        }
        if (address(pairPangolin) == address(0) && address(pairRip) == address(0)) {
            return 0;
        }
        ...
        if (address(pairRip) != address(0)) {
            (uint112 reserve0Rip, uint112 reserve1Rip, ) = pairRip.getReserves();
            reserve0 += reserve0Rip;
            reserve1 += reserve1Rip;
            if (token0 == IERC20(0)) {
                token0 = pairRip.token0();
            }
        }
    ...
    struct UIInfo {
        uint256 avaxBalance;
        uint256 RipBalance;
        uint256 RipBarBalance;
        uint256 xRipBalance;
        uint256 xRipSupply;
        uint256 RipBarAllowance;
        Factory[] factories;
        uint256 avaxRate;
        uint256 RipRate;
        uint256 btcRate;
    ...
    function getUIInfo(
        ...
        if (Rip != IERC20(0)) {
            info.RipRate = getAVAXRate(Rip);
            info.RipBalance = Rip.balanceOf(who);
            info.RipBarBalance = Rip.balanceOf(address(bar));
            info.RipBarAllowance = Rip.allowance(who, address(bar));
        }

        if (bar != IERC20(0)) {
            info.xRipBalance = bar.balanceOf(who);
            info.xRipSupply = bar.totalSupply();
        }
        ...
        if (chef != IMasterChef(0)) {
            ...
            for (uint256 i = 0; i < poolLength; i++) {
                (uint256 pendingRipAmt, , , ) = chef.pendingTokens(i, who);
                pendingRip += pendingRipAmt;
            }
            info.pendingRip = pendingRip;
        }
    ...
    function pollPools(address who, uint256[] calldata pids) public view returns (UserPoolInfo[] memory) {
        ...
        for (uint256 i = 0; i < pids.length; i++) {
            ...
            (uint256 pendingRip, , , ) = chef.pendingTokens(pids[i], who);
            pools[i].pending = pendingRip;
