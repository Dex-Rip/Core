...
contract BoringCryptoDashboardV2 {
    ...
    constructor(
        ...
        RipFactory = IFactory(_RipFactory);
        wavax = _wavax;
    }
    ...
    function getAVAXRate(address token) public view returns (uint256) {
        ...
        if (token != wavax) {
            ...
            pairRip = IPair(IFactory(RipFactory).getPair(token, wavax));
            ...
            if (address(pairRip) != address(0)) {
                (reserve0Rip, reserve1Rip, ) = pairRip.getReserves();
            }

            if (address(pairRip) == address(0) || reserve0Pangolin > reserve0Rip || reserve1Pangolin > reserve1Rip) {
                ...
                if (pairRip.token0() == wavax) {
                    avax_rate = uint256(reserve1Rip).mul(1e18).div(reserve0Rip);
                } else {
                    avax_rate = uint256(reserve0Rip).mul(1e18).div(reserve1Rip);
                }
            }
        }
        return avax_rate;
